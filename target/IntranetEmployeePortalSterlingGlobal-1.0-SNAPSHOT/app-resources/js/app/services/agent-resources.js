...
app.factory('UpdatetingArticle', ['$resource', '$window', '$stateParams', function($resource, $window, $stateParams)
{
    var id = $stateParams.draftArticleId;
    var urlBase = $window.location.origin + $window.location.pathname + '/update-article/' + id;

    return $resource(urlBase + ':articleId', {draftArticleId: '@articleId'}, {
...
app.factory('UpdatetingArticleServices', ['$http', '$window', function($http, $window)
{

    var updateArticle = {}

    updateArticle.fetchArticle = function(articleId, urlBase, cb)
    {
